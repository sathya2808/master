...
sudo /bin/systemctl stop responsible-game-play
sed -i 's/circapp/stack_id/g' /usr/local/scripts/copytos3.sh 
sed -i 's/circapp/stack_id/g' /usr/local/scripts/logrotate.sh 


sed -i 's/circdb/stack_id/g' /home/deploy/apps/responsible-game-play/pt/*.properties

sudo /bin/systemctl start responsible-game-play
